...
def solve_puzzle_parallel(
    ...
    # spawn context required for Windows compatibility
    for i, (seed, start, end) in enumerate(ranges):
        result = solve_chunk((seed, start, end))
...
def save_skill(seed: bytes, contract_address: str) -> None:
    """
    Thread-safe update of puzzle seed in skill.md.
    ...
    while lock_file.exists():
        ...
        waited += 1
        if waited > 50:
            print("[Skill] [WARN]️  Lock timeout — writing anyway")
            break
...
def main() -> None:
    ...
    w3.middleware_onion.add(
        __import__("web3.middleware", fromlist=["SignAndSendRawMiddlewareBuilder"]).SignAndSendRawMiddlewareBuilder.build(wallet)
    )
